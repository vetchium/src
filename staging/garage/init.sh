#!/bin/sh
# Idempotent bootstrap for ONE Garage node. Runs in that node's network namespace
# (network_mode: service:garage-<region>) with the node's metadata mounted, so the
# CLI talks to it over localhost via the config (no node-id wrangling needed).
# Assigns a single-node layout, imports the fixed S3 key, creates the region's
# bucket(s), grants access. Re-runnable (skips already-configured steps).
set -eu
