...
export const MyInterviewsPage: React.FC = () => {
	...
	const [interviews, setInterviews] = useState<HubMyInterview[]>([]);
	...
	const fetchInterviews = useCallback(
		async (paginationKey?: string) => {
			...
			try {
				...
				const res = await fetch(`${apiBaseUrl}/hub/list-my-interviews`, {
					method: "POST",
					headers: {
	...
	const columns = [
		{
			...
			key: "opening_title",
			render: (title: string, record: HubMyInterview) => (
		...
		},
		{
			...
			key: "interview_type",
			render: (type: string) => t(`type_${type}` as "type_video"),
		},
		{
			title: t("when"),
			dataIndex: "starts_at",
