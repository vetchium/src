...
const STATE_COLORS: Record<string, string> = {
	...
	candidate_not_responding: "default",
	employer_defunct: "default",
};

export const MyCandidaciesPage: React.FC = () => {
	const { t, i18n } = useTranslation("candidacies");
	const { sessionToken } = useAuth();
	const navigate = useNavigate();
	const [candidacies, setCandidacies] = useState<HubCandidacySummary[]>([]);
	...
	const fetchCandidacies = useCallback(
		async (paginationKey?: string) => {
			...
			try {
				...
				const req: ListMyCandidaciesRequest = {
					limit: 20,
					...(paginationKey ? { pagination_key: paginationKey } : {}),
				};
				const res = await fetch(`${apiBaseUrl}/hub/list-my-candidacies`, {
					method: "POST",
					headers: {
				...
				if (res.status === 200) {
					const data: ListMyCandidaciesResponse = await res.json();
					setCandidacies((prev) =>
	...
	const columns = [
		{
			...
			key: "opening_title",
			render: (title: string, record: HubCandidacySummary) => (
				<Link to={`/my-candidacies/${record.candidacy_id}`}>{title}</Link>
			),
		},
		{
			title: t("company"),
			key: "org",
			render: (_: unknown, record: HubCandidacySummary) =>
		...
		},
		{
			...
			key: "actions",
			render: (_: unknown, record: HubCandidacySummary) => (
				<Link to={`/my-candidacies/${record.candidacy_id}`}>{t("view")}</Link>
			),
		},
	];
			...
			<Spin spinning={loading}>
				{candidacies.length === 0 && !loading ? (
					...
					<Table
						...
						pagination={false}
						onRow={(record) => ({
							onClick: () => navigate(`/my-candidacies/${record.candidacy_id}`),
