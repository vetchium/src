...
export function validateRequestPasswordResetRequest(
  ...
  return isEmailAddress(
    normalizeRequestPasswordResetRequest(request).email_address,
  )
    ? []
    : ["email_address"];
}
...
export function validateCompletePasswordResetRequest(
  ...
  const fields: string[] = [];
  if (!isOpaqueToken(request.reset_token)) {
    fields.push("reset_token");
  }
  if (!isNewPassword(request.new_password)) {
    fields.push("new_password");
  }
  return fields;
}
