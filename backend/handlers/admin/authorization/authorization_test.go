...
) (http.Handler, *adminruntime.Server) {
	server := handlertest.Server(db, now)
...
func TestSetPermissionsStoresGrantsWithoutImpliedPermissions(t *testing.T) {
	...
	response := performSetPermissions(handler, `{"admin_user_id":"`+
		handlertest.AdminUserID+`","permissions":["admin:view_users",`+
		`"admin:manage_users"]}`)
...
func TestSetPermissionsClearsEveryGrant(t *testing.T) {
	...
	response := performSetPermissions(
		handler,
		`{"admin_user_id":"`+handlertest.AdminUserID+`","permissions":[]}`,
	)
...
func TestSetPermissionsFailureResponses(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			permissions: []string{"admin:manage_users"},
			body: `{"admin_user_id":"` + handlertest.AdminUserID +
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			details := handlertest.DecodeProblem(t, response)
...
func TestSetPermissionsRequiresAuthentication(t *testing.T) {
	...
	request := httptest.NewRequest(
		http.MethodPost, "/api/admin/set-user-permissions",
		bytes.NewBufferString(`{"admin_user_id":"`+handlertest.AdminUserID+
			`","permissions":[]}`),
	)
	request.Header.Set("Content-Type", "application/json")
	response := httptest.NewRecorder()
	...
	if got := response.Header().Get("WWW-Authenticate"); got != adminauthn.BearerChallenge {
		t.Fatalf("WWW-Authenticate = %q, want %q", got, adminauthn.BearerChallenge)
	}
	details := handlertest.DecodeProblem(t, response)
