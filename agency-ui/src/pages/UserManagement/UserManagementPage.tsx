...
import type {
	AgencyUser,
	FilterAgencyUsersRequest,
	FilterAgencyUsersResponse,
...
export function UserManagementPage() {
	...
	// Permission logic for Agency portal: is_admin OR has specific role
	const canInviteUsers =
		myInfo?.is_admin || myInfo?.roles.includes("agency:invite_users") || false;
	...
	const fetchUsers = useCallback(
		async (
			...
			try {
				...
				const requestBody: FilterAgencyUsersRequest = {
					limit: 50,
				};
				...
				const response = await fetch(`${apiBaseUrl}/agency/filter-users`, {
					method: "POST",
					headers: {
				...
				const data: FilterAgencyUsersResponse = await response.json();
	...
	const handleViewDetails = (user: AgencyUser) => {
	...
	const columns = [
		...
		},
		{
			...
			key: "actions",
			render: (_: unknown, record: AgencyUser) => (
				<Space>
					<Button
						type="link"
						size="small"
