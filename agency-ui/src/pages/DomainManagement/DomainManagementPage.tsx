...
import { AgencyDomainVerificationStatus } from "vetchium-specs/agency-domains/agency-domains";
import {
	DOMAIN_MAX_LENGTH,
	DOMAIN_MIN_LENGTH,
...
export function DomainManagementPage() {
	...
	const [instructionsDomain, setInstructionsDomain] =
		useState<AgencyListDomainStatusItem | null>(null);
	...
	const handleRequestVerification = async (domain: string) => {
		...
		try {
			...
			if (response.status === 200) {
				...
				if (data.status === AgencyDomainVerificationStatus.VERIFIED) {
	...
	const columns = [
		...
		},
		{
			...
			render: (status: string) => {
				switch (status) {
					case AgencyDomainVerificationStatus.VERIFIED:
						return (
							<Tag icon={<CheckCircleOutlined />} color="success">
								{t("domain.statusVerified")}
							</Tag>
						);
					case AgencyDomainVerificationStatus.PENDING:
						return (
							<Tag icon={<ClockCircleOutlined />} color="processing">
								{t("domain.statusPending")}
							</Tag>
						);
					case AgencyDomainVerificationStatus.FAILING:
						return (
		...
		},
		{
			...
			key: "info",
			render: (_: unknown, record: AgencyListDomainStatusItem) => {
				if (record.status === AgencyDomainVerificationStatus.VERIFIED) {
					return (
