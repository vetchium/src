...
export function AuthProvider({ children }: AuthProviderProps) {
	...
	const login = useCallback(
		async (email: string, domain: string, password: string) => {
			...
			try {
				...
				const response = await fetch(`${apiBaseUrl}/agency/login`, {
					method: "POST",
	...
	const verifyTFA = useCallback(
		async (tfaCode: string, rememberMe: boolean) => {
			...
			const validationErrors = validateAgencyTFARequest(tfaRequest);
			...
			try {
				const apiBaseUrl = await getApiBaseUrl();
				const response = await fetch(`${apiBaseUrl}/agency/tfa`, {
					method: "POST",
					headers: { "Content-Type": "application/json" },
	...
	const logout = useCallback(async () => {
		...
		try {
			const apiBaseUrl = await getApiBaseUrl();
			const response = await fetch(`${apiBaseUrl}/agency/logout`, {
				method: "POST",
				headers: {
