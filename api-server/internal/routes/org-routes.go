...
func RegisterOrgRoutes(mux *http.ServeMux, s *server.RegionalServer) {
	...
	mux.Handle("POST /org/list-client-default-recruiters", orgAuth(orgRoleViewAgencyReferrals(org.ListClientDefaultRecruiters(s))))
	mux.Handle("POST /org/assign-opening-recruiters", orgAuth(orgRoleManageAgencyRecruiters(org.AssignOpeningRecruiters(s))))
	mux.Handle("POST /org/remove-opening-recruiter", orgAuth(orgRoleManageAgencyRecruiters(org.RemoveOpeningRecruiter(s))))
	mux.Handle("POST /org/set-client-default-recruiters", orgAuth(orgRoleManageAgencyRecruiters(org.SetClientDefaultRecruiters(s))))
	mux.Handle("POST /org/remove-client-default-recruiter", orgAuth(orgRoleManageAgencyRecruiters(org.RemoveClientDefaultRecruiter(s))))
