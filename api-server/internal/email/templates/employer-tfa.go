...
const nsEmployerTFA = "emails/employer_tfa"

// EmployerTFAData contains data for the org user TFA email
type EmployerTFAData struct {
	Code    string // 6-digit verification code
	Minutes int    // Expiry time in minutes
}

// EmployerTFASubject returns the localized email subject for org user TFA
func EmployerTFASubject(lang string) string {
	return i18n.T(lang, nsEmployerTFA, "subject")
}

// EmployerTFATextBody returns the localized plain text body for org user TFA email
func EmployerTFATextBody(lang string, data EmployerTFAData) string {
	portalName := i18n.T(lang, nsEmployerTFA, "portal_name")
	intro := i18n.T(lang, nsEmployerTFA, "body_intro")
	expiry := i18n.TF(lang, nsEmployerTFA, "body_expiry", data)
	ignore := i18n.T(lang, nsEmployerTFA, "body_ignore")
	footer := i18n.T(lang, nsEmployerTFA, "footer")
...
// EmployerTFAHTMLBody returns the localized HTML body for org user TFA email
func EmployerTFAHTMLBody(lang string, data EmployerTFAData) string {
	escapedCode := html.EscapeString(data.Code)
	portalName := html.EscapeString(i18n.T(lang, nsEmployerTFA, "portal_name"))
	intro := html.EscapeString(i18n.T(lang, nsEmployerTFA, "body_intro"))
	expiry := html.EscapeString(i18n.TF(lang, nsEmployerTFA, "body_expiry", data))
	ignore := html.EscapeString(i18n.T(lang, nsEmployerTFA, "body_ignore"))
	footer := html.EscapeString(i18n.T(lang, nsEmployerTFA, "footer"))
