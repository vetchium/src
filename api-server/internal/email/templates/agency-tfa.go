...
const nsAgencyTFA = "emails/agency_tfa"

// AgencyTFAData contains data for the agency user TFA email
type AgencyTFAData struct {
	Code    string // 6-digit verification code
	Minutes int    // Expiry time in minutes
}

// AgencyTFASubject returns the localized email subject for agency user TFA
func AgencyTFASubject(lang string) string {
	return i18n.T(lang, nsAgencyTFA, "subject")
}

// AgencyTFATextBody returns the localized plain text body for agency user TFA email
func AgencyTFATextBody(lang string, data AgencyTFAData) string {
	portalName := i18n.T(lang, nsAgencyTFA, "portal_name")
	intro := i18n.T(lang, nsAgencyTFA, "body_intro")
	expiry := i18n.TF(lang, nsAgencyTFA, "body_expiry", data)
	ignore := i18n.T(lang, nsAgencyTFA, "body_ignore")
	footer := i18n.T(lang, nsAgencyTFA, "footer")
...
// AgencyTFAHTMLBody returns the localized HTML body for agency user TFA email
func AgencyTFAHTMLBody(lang string, data AgencyTFAData) string {
	escapedCode := html.EscapeString(data.Code)
	portalName := html.EscapeString(i18n.T(lang, nsAgencyTFA, "portal_name"))
	intro := html.EscapeString(i18n.T(lang, nsAgencyTFA, "body_intro"))
	expiry := html.EscapeString(i18n.TF(lang, nsAgencyTFA, "body_expiry", data))
	ignore := html.EscapeString(i18n.T(lang, nsAgencyTFA, "body_ignore"))
	footer := html.EscapeString(i18n.T(lang, nsAgencyTFA, "footer"))
