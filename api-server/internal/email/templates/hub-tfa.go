...
const nsHubTFA = "emails/hub_tfa"

// HubTFAData contains data for the hub user TFA email
type HubTFAData struct {
	Code    string // 6-digit verification code
	Minutes int    // Expiry time in minutes
}

// HubTFASubject returns the localized email subject for hub user TFA
func HubTFASubject(lang string) string {
	return i18n.T(lang, nsHubTFA, "subject")
}

// HubTFATextBody returns the localized plain text body for hub user TFA email
func HubTFATextBody(lang string, data HubTFAData) string {
	portalName := i18n.T(lang, nsHubTFA, "portal_name")
	intro := i18n.T(lang, nsHubTFA, "body_intro")
	expiry := i18n.TF(lang, nsHubTFA, "body_expiry", data)
	ignore := i18n.T(lang, nsHubTFA, "body_ignore")
	footer := i18n.T(lang, nsHubTFA, "footer")
...
// HubTFAHTMLBody returns the localized HTML body for hub user TFA email
func HubTFAHTMLBody(lang string, data HubTFAData) string {
	escapedCode := html.EscapeString(data.Code)
	portalName := html.EscapeString(i18n.T(lang, nsHubTFA, "portal_name"))
	intro := html.EscapeString(i18n.T(lang, nsHubTFA, "body_intro"))
	expiry := html.EscapeString(i18n.TF(lang, nsHubTFA, "body_expiry", data))
	ignore := html.EscapeString(i18n.T(lang, nsHubTFA, "body_ignore"))
	footer := html.EscapeString(i18n.T(lang, nsHubTFA, "footer"))
