...
func Login(s *server.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		regionalUser, err := s.Regional.GetAgencyUserByEmailAndAgency(ctx, regionaldb.GetAgencyUserByEmailAndAgencyParams{
		...
		// Store TFA token and enqueue email atomically in regional database
		...
		err = s.WithRegionalTx(ctx, func(qtx *regionaldb.Queries) error {
			txErr := qtx.CreateAgencyTFAToken(ctx, regionaldb.CreateAgencyTFATokenParams{
