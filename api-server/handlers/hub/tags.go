...
func buildTagFromLocaleRow(row globaldb.GetTagWithLocaleRow) hub.Tag {
	t := hub.Tag{
		TagID:       row.TagID,
		DisplayName: row.DisplayName,
	}
...
// GetTag handles POST /hub/get-tag
...
// FilterTags handles POST /hub/filter-tags
func FilterTags(s *server.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		tags := make([]hub.Tag, 0, len(rows))
		for _, row := range rows {
			tags = append(tags, buildTagResponse(row))
		}

		json.NewEncoder(w).Encode(hub.FilterTagsResponse{
			Tags:          tags,
