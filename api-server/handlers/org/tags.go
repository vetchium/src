...
func buildTagResponse(row globaldb.FilterTagsForLocaleRow) org.Tag {
	t := org.Tag{
		TagID:       row.TagID,
...
func buildTagFromLocaleRow(row globaldb.GetTagWithLocaleRow) org.Tag {
	t := org.Tag{
		TagID:       row.TagID,
		DisplayName: row.DisplayName,
	}
...
// GetTag handles POST /org/get-tag
...
// FilterTags handles POST /org/filter-tags
func FilterTags(s *server.RegionalServer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		tags := make([]org.Tag, 0, len(rows))
		for _, row := range rows {
			tags = append(tags, buildTagResponse(row))
		}

		json.NewEncoder(w).Encode(org.FilterTagsResponse{
			Tags:          tags,
