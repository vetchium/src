...
func buildTagResponse(row globaldb.FilterTagsForLocaleRow) employer.Tag {
	t := employer.Tag{
		TagID:       row.TagID,
...
func buildTagFromLocaleRow(row globaldb.GetTagWithLocaleRow) employer.Tag {
	t := employer.Tag{
		TagID:       row.TagID,
		DisplayName: row.DisplayName,
	}
...
// GetTag handles POST /employer/get-tag
func GetTag(s *server.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req employer.GetTagRequest
...
// FilterTags handles POST /employer/filter-tags
func FilterTags(s *server.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req employer.FilterTagsRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			log.Debug("failed to decode request", "error", err)
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		...
		tags := make([]employer.Tag, 0, len(rows))
		for _, row := range rows {
			tags = append(tags, buildTagResponse(row))
		}

		json.NewEncoder(w).Encode(employer.FilterTagsResponse{
			Tags:          tags,
