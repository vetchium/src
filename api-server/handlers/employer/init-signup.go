...
func InitSignup(s *server.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req employer.OrgInitSignupRequest
		...
		if err == nil {
			...
			s.Logger(ctx).Error("failed to check existing org user", "error", err)
		...
		if err == nil {
			s.Logger(ctx).Debug("domain already claimed by existing employer", "domain", domain)
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode([]common.ValidationError{
			...
			s.Logger(ctx).Error("failed to query global employer domain", "error", err)
			http.Error(w, "", http.StatusInternalServerError)
			return
		}
		...
		s.Logger(ctx).Info("org signup emails sent (DNS + token)", "email_hash", hex.EncodeToString(emailHash[:]), "domain", domain)
		...
		response := employer.OrgInitSignupResponse{
...
func sendOrgSignupDNSEmail(ctx context.Context, db *regionaldb.Queries, to string, domain string, dnsRecordName string, dnsRecordValue string, lang string, expiryHours int) error {
	data := templates.EmployerSignupData{
		Domain:         domain,
		DNSRecordName:  dnsRecordName,
	...
	_, err := db.EnqueueEmail(ctx, regionaldb.EnqueueEmailParams{
		...
		EmailTo:       to,
		EmailSubject:  templates.EmployerSignupSubject(lang),
		EmailTextBody: templates.EmployerSignupTextBody(lang, data),
		EmailHtmlBody: templates.EmployerSignupHTMLBody(lang, data),
	})
	return err
}
...
func sendOrgSignupTokenEmail(ctx context.Context, db *regionaldb.Queries, to string, domain string, signupToken string, signupLink string, lang string, expiryHours int) error {
	data := templates.EmployerSignupTokenData{
		Domain:      domain,
	...
	_, err := db.EnqueueEmail(ctx, regionaldb.EnqueueEmailParams{
		...
		EmailTo:       to,
		EmailSubject:  templates.EmployerSignupTokenSubject(lang),
		EmailTextBody: templates.EmployerSignupTokenTextBody(lang, data),
		EmailHtmlBody: templates.EmployerSignupTokenHTMLBody(lang, data),
	})
	return err
}
