...
func TFA(s *server.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var tfaRequest employertypes.OrgTFARequest
		...
		if err != nil {
			s.Logger(ctx).Error("failed to fetch employer", "error", err)
			http.Error(w, "", http.StatusInternalServerError)
			return
		}
		...
		response := employertypes.OrgTFAResponse{
			SessionToken:      employertypes.OrgSessionToken(sessionToken),
