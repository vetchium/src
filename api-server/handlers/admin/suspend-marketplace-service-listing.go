...
// SuspendMarketplaceServiceListing handles POST /admin/suspend-marketplace-service-listing
func SuspendMarketplaceServiceListing(s *server.GlobalServer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req admintypes.AdminSuspendMarketplaceServiceListingRequest
		...
		err := s.WithRegionalTx(ctx, region, func(qtx *regionaldb.Queries) error {
			var txErr error
			updatedListing, txErr = qtx.AdminSuspendServiceListing(ctx, regionaldb.AdminSuspendServiceListingParams{
		...
		if err != nil {
			...
			log.Error("failed to suspend service listing", "error", err)
			http.Error(w, "", http.StatusInternalServerError)
			return
		}
		...
		auditErr := s.WithGlobalTx(ctx, func(qtx *globaldb.Queries) error {
			return qtx.InsertAdminAuditLog(ctx, globaldb.InsertAdminAuditLogParams{
				EventType:   "admin.suspend_marketplace_service_listing",
		...
		if auditErr != nil {
			log.Error("CONSISTENCY_ALERT: failed to write audit log after successful service listing suspension",
				"error", auditErr, "service_listing_id", req.ServiceListingID)
		}

		log.Info("marketplace service listing suspended", "service_listing_id", req.ServiceListingID, "admin_id", uuidToString(adminUser.AdminUserID))
