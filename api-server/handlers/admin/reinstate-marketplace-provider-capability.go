...
// ReinstateMarketplaceProviderCapability handles POST /admin/reinstate-marketplace-provider-capability
// Reinstates a capability that was revoked (WHERE status = 'revoked').
func ReinstateMarketplaceProviderCapability(s *server.GlobalServer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req admintypes.ReinstateMarketplaceProviderCapabilityRequest
		...
		err = s.WithRegionalTx(ctx, org.Region, func(qtx *regionaldb.Queries) error {
			var txErr error
			updatedCap, txErr = qtx.AdminReinstateOrgCapability(ctx, regionaldb.AdminReinstateOrgCapabilityParams{
		...
		if err != nil {
			...
			log.Error("failed to reinstate org capability", "error", err)
			http.Error(w, "", http.StatusInternalServerError)
			return
		}
		...
		auditErr := s.WithGlobalTx(ctx, func(qtx *globaldb.Queries) error {
			return qtx.InsertAdminAuditLog(ctx, globaldb.InsertAdminAuditLogParams{
				EventType:   "admin.reinstate_marketplace_provider_capability",
		...
		if auditErr != nil {
			log.Error("CONSISTENCY_ALERT: failed to write audit log after successful regional capability reinstatement",
				"error", auditErr, "org_domain", req.OrgDomain)
		}

		log.Info("marketplace provider capability reinstated", "org_domain", req.OrgDomain, "admin_id", uuidToString(adminUser.AdminUserID))
