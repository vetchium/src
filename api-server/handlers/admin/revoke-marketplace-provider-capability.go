...
// RevokeMarketplaceProviderCapability handles POST /admin/revoke-marketplace-provider-capability
func RevokeMarketplaceProviderCapability(s *server.GlobalServer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req admintypes.RevokeMarketplaceProviderCapabilityRequest
		...
		err = s.WithRegionalTx(ctx, org.Region, func(qtx *regionaldb.Queries) error {
			var txErr error
			updatedCap, txErr = qtx.AdminRevokeOrgCapability(ctx, regionaldb.AdminRevokeOrgCapabilityParams{
		...
		if err != nil {
			...
			log.Error("failed to revoke org capability", "error", err)
			http.Error(w, "", http.StatusInternalServerError)
			return
		}
		...
		auditErr := s.WithGlobalTx(ctx, func(qtx *globaldb.Queries) error {
			return qtx.InsertAdminAuditLog(ctx, globaldb.InsertAdminAuditLogParams{
				EventType:   "admin.revoke_marketplace_provider_capability",
		...
		if auditErr != nil {
			log.Error("CONSISTENCY_ALERT: failed to write audit log after successful regional capability revocation",
				"error", auditErr, "org_id", req.OrgID)
		}

		log.Info("marketplace provider capability revoked", "org_id", req.OrgID, "admin_id", uuidToString(adminUser.AdminUserID))
