...
import { createTestOrgAdminDirect, deleteTestOrgByDomain, deleteTestGlobalOrgDomain } from "../../../lib/db";
...
test.describe("Org UI - Domain Management", () => {
    ...
    test("should claim a new domain and see DNS instructions", async ({ page }) => {
        ...
        await expect(page.locator(".ant-alert-success")).toContainText("DNS Configuration");
        await expect(page.locator(".ant-alert-success")).toContainText(newDomain);

        // Verify it appears in the table
        await expect(page.locator('table').locator(`text=${newDomain}`)).toBeVisible();
