...
import { permanentlyDeleteTestApprovedDomain, createTestSuperadmin, deleteTestAdminUser } from "../../../lib/db";
...
test.describe("Admin UI - Approved Domains", () => {
    ...
    test("should add, filter, and disable an approved domain", async ({ page }) => {
        ...
        await page.fill('textarea[placeholder="Enter the reason for adding this domain..."]', "UI Testing auto-add");
        await page.click('.ant-modal-footer button:has-text("Add")');

        // Verify success message
        await expect(page.locator(".ant-message-success").filter({ hasText: "Domain added successfully" })).toBeVisible();
        ...
        await page.fill('textarea[placeholder="Enter the reason for disabling this domain..."]', "UI Testing auto-disable");
        await page.click('.ant-modal-footer button:has-text("Disable")');

        // Verify success
        await expect(page.locator(".ant-message-success").filter({ hasText: "Domain disabled successfully" })).toBeVisible();

        // Verify it's gone from Active tab
        await expect(page.locator('table').locator(`text=${domainName}`)).not.toBeVisible();

        // Switch to Inactive tab and verify it's there
        await page.click('.ant-tabs-tab-btn:has-text("Inactive")');
        await expect(page.locator('table').locator(`text=${domainName}`)).toBeVisible();
    });
});
