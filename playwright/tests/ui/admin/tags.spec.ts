...
test.describe("Admin UI - Manage Tags", () => {
	...
	test("should create and search for a tag", async ({ page }) => {
		...
		// Fill Add Tag Modal
		await modal
			.locator('input[placeholder="e.g. artificial-intelligence"]')
			.fill(tagId);
		...
		await expect(
			page
				...
				.filter({ hasText: "Tag created successfully" })
		).toBeVisible();
		...
		await expect(
			page.locator("tbody tr").filter({ hasText: tagId })
		).toBeVisible();
		await expect(
			page.locator("tbody tr").filter({ hasText: "Test Tag UI Name" })
		).toBeVisible();
	});
});
