...
test.describe("Email i18n - Language-specific email content", () => {
  ...
  test.describe("English (en-US)", () => {
    test("TFA email subject is in English for en-US user", async ({ request }) => {
      const api = new AdminAPIClient(request);
      const email = generateTestEmail("i18n-en-subject");
      const password = "ValidPassword123$";

      await createTestAdminUser(email, password, { preferredLanguage: "en-US" });
      try {
    ...
    test("TFA email HTML body is in English for en-US user", async ({ request }) => {
      const api = new AdminAPIClient(request);
  ...
  test.describe("German (de-DE)", () => {
    test("TFA email subject is in German for de-DE user", async ({ request }) => {
      const api = new AdminAPIClient(request);
    ...
    test("TFA email HTML body has German lang attribute for de-DE user", async ({ request }) => {
      const api = new AdminAPIClient(request);
  ...
  test.describe("Tamil (ta-IN)", () => {
    test("TFA email subject is in Tamil for ta-IN user", async ({ request }) => {
      const api = new AdminAPIClient(request);
    ...
    test("TFA email HTML body has Tamil lang attribute for ta-IN user", async ({ request }) => {
      const api = new AdminAPIClient(request);
  ...
  test.describe("Language Fallback", () => {
    test("unsupported language falls back to English (fr-FR)", async ({ request }) => {
      const api = new AdminAPIClient(request);
      const email = generateTestEmail("i18n-fallback-fr");
      const password = "ValidPassword123$";

      // French is not supported, should fall back to English
      await createTestAdminUser(email, password, { preferredLanguage: "fr-FR" });
      try {
        const response = await api.login(email, password);
        expect(response.status).toBe(200);
    ...
    test("unsupported language falls back to English (ja-JP)", async ({ request }) => {
      const api = new AdminAPIClient(request);
      const email = generateTestEmail("i18n-fallback-ja");
      const password = "ValidPassword123$";

      // Japanese is not supported, should fall back to English
      await createTestAdminUser(email, password, { preferredLanguage: "ja-JP" });
      try {
        const response = await api.login(email, password);
        expect(response.status).toBe(200);
    ...
    test("unsupported regional variant falls back to base language (en-GB to en-US)", async ({
      ...
      await createTestAdminUser(email, password, { preferredLanguage: "en-GB" });
      try {
        const response = await api.login(email, password);
        expect(response.status).toBe(200);
    ...
    test("unsupported German variant falls back to de-DE (de-AT)", async ({ request }) => {
      const api = new AdminAPIClient(request);
      const email = generateTestEmail("i18n-fallback-de-at");
      const password = "ValidPassword123$";

      // Austrian German (de-AT) not supported, should fall back to de-DE
      await createTestAdminUser(email, password, { preferredLanguage: "de-AT" });
      try {
        const response = await api.login(email, password);
        expect(response.status).toBe(200);
    ...
    test("invalid language code falls back to English", async ({ request }) => {
      ...
      await createTestAdminUser(email, password, { preferredLanguage: "xyz-123" });
      try {
        const response = await api.login(email, password);
        expect(response.status).toBe(200);
  ...
  test.describe("Default Language", () => {
    test("user created without explicit language gets English emails", async ({ request }) => {
      const api = new AdminAPIClient(request);
    ...
    test("user created with empty options object gets English emails", async ({ request }) => {
      const api = new AdminAPIClient(request);
  ...
  test.describe("Email Content Verification", () => {
    test("TFA code is present in localized English email", async ({ request }) => {
      const api = new AdminAPIClient(request);
    ...
    test("TFA code is present in localized German email", async ({ request }) => {
      const api = new AdminAPIClient(request);
    ...
    test("TFA code is present in localized Tamil email", async ({ request }) => {
      const api = new AdminAPIClient(request);
    ...
    test("expiry time is mentioned in all languages", async ({ request }) => {
      ...
      const testCases = [
        { email: generateTestEmail("i18n-expiry-en"), lang: "en-US" as LanguageCode, expectedPattern: /10 minutes/ },
        { email: generateTestEmail("i18n-expiry-de"), lang: "de-DE" as LanguageCode, expectedPattern: /10 Minuten/ },
        { email: generateTestEmail("i18n-expiry-ta"), lang: "ta-IN" as LanguageCode, expectedPattern: /10 நிமிடங்களில்/ },
      ];

      // Create all users first
      for (const testCase of testCases) {
        await createTestAdminUser(testCase.email, password, { preferredLanguage: testCase.lang });
      }

      try {
        // Login all concurrently
        const loginPromises = testCases.map((tc) => api.login(tc.email, password));
        ...
        const fullEmailPromises = emailSummaries.map((summary) => getEmailContent(summary.ID));
  ...
  test.describe("Parallel Execution Isolation", () => {
    ...
    test("concurrent logins with different languages receive correct emails", async ({
      ...
      const users = [
        { email: generateTestEmail("i18n-parallel-en"), lang: "en-US" as LanguageCode },
        { email: generateTestEmail("i18n-parallel-de"), lang: "de-DE" as LanguageCode },
        { email: generateTestEmail("i18n-parallel-ta"), lang: "ta-IN" as LanguageCode },
      ];

      // Create all users
      for (const user of users) {
        await createTestAdminUser(user.email, password, { preferredLanguage: user.lang });
      }

      try {
        // Login all users concurrently
        const loginPromises = users.map((user) => api.login(user.email, password));
        const responses = await Promise.all(loginPromises);
