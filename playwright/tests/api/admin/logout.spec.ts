...
async function getSessionToken(
  ...
  const tfaCode = await getTfaCodeFromEmail(email, 10000);
...
test.describe("POST /admin/logout", () => {
  ...
  test("malformed session token returns 400", async ({ request }) => {
    const api = new AdminAPIClient(request);
    ...
    const response2 = await api.logoutRaw({
      session_token: "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz",
    });
