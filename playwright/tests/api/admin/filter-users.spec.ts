...
test.describe("Admin Filter Users API", () => {
    ...
    test.beforeAll(async ({ request }) => {
        ...
        await createTestAdminUser(mainAdminEmail, TEST_PASSWORD, { status: "active", fullName: "Main Admin" });
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            await createTestAdminUser(email, TEST_PASSWORD, { status: "active", fullName: `Filter Target ${i}` });
            testUsers.push(email);
        }
        // Create one disabled user
        const disabledEmail = generateTestEmail("filter-disabled");
        await createTestAdminUser(disabledEmail, TEST_PASSWORD, { status: "disabled", fullName: "Disabled User" });
    ...
    test("should filter users by partial email", async ({ request }) => {
        ...
        const matching = items.filter(u => u.email_address.includes("filter-target"));
    ...
    test("should filter users by partial name", async ({ request }) => {
        ...
        expect(response.status).toBe(200);
        const matching = response.body!.items.filter(u => u.name.includes("Filter Target"));
        expect(matching.length).toBeGreaterThanOrEqual(3);
    });
    ...
    test("should support keyset pagination", async ({ request }) => {
        ...
        expect(res2.body!.items[0].email_address).not.toBe(res1.body!.items[1].email_address);
    });

    test("should return empty list for non-matching filter", async ({ request }) => {
        const adminApiClient = new AdminAPIClient(request);
        const response = await adminApiClient.filterUsers(mainAdminToken, {
