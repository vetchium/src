...
import { createTestAgencyUserDirect, deleteTestAgencyUser, generateTestAgencyEmail } from "../../../lib/db";
...
test.describe("Agency Filter Users API", () => {
    ...
    test.beforeAll(async ({ request }) => {
        ...
        const mainUser = await createTestAgencyUserDirect(email, TEST_PASSWORD, "ind1");
    ...
    test("should filter users by partial email", async ({ request }) => {
        ...
        expect(response.status).toBe(200);
        const matching = response.body!.items.filter(u => u.email_address.includes("user"));
    ...
    test("should filter users by status", async ({ request }) => {
        ...
        expect(response.status).toBe(200);
        const matching = response.body!.items.filter(u => u.status === "disabled");
