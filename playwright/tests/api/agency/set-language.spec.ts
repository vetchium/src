...
import type { AgencySetLanguageRequest } from "vetchium-specs/agency/agency-users";
...
test.describe("Agency Set Language API", () => {
	let email: string;
	let domainName: string;
	let password: string;
	let sessionToken: string;
	let api: AgencyAPIClient;

	test.beforeEach(async ({ request }) => {
		api = new AgencyAPIClient(request);
		password = TEST_PASSWORD;
		const testEmail = generateTestAgencyEmail("agency-lang");
