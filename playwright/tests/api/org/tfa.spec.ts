...
test.describe("POST /org/tfa", () => {
	test("successful TFA verification returns session token and records org.login event", async ({
		request,
	}) => {
		...
		try {
			...
			expect(response.body.org_name).toBeDefined();
			...
			expect(auditResp.body.audit_logs[0].event_type).toBe("org.login");
		} finally {
