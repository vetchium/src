...
test.describe("POST /org/logout", () => {
	test("successful logout returns 200 and records org.logout event", async ({
		request,
	}) => {
		...
		try {
			...
			expect(auditResp.body.audit_logs[0].event_type).toBe("org.logout");
		} finally {
