...
expect(res.body!.openings.some((o) => o.title === "Frontend Engineer")).toBe(
	true
);
expect(res.body!.openings.some((o) => o.title === "Frontend Designer")).toBe(
	false
);
...
const res = await api.listOpenings(token, {
	filter_hiring_manager_email_address: recruiterEmail,
});
expect(res.status).toBe(200);
expect(res.body!.openings.some((o) => o.title === "Backend Engineer")).toBe(
	true
...
```ts
const res = await api.listOpenings(token, {
	filter_recruiter_email_address: adminEmail,
});
expect(res.status).toBe(200);
expect(res.body!.openings.some((o) => o.title === "Backend Engineer")).toBe(
	true
...
const req: CreateOpeningRequest = {
	...baseFields,
	internal_notes: "Only for internal use",
};
...
```ts
const ccRes = await api.addCostCenter(token, {
	name: "Engineering",
	notes: "Eng team",
});
const req: CreateOpeningRequest = {
	...baseFields,
	cost_center_id: ccRes.body!.cost_center_id,
};
...
```ts
await expect(
	page.locator('button:has-text("Create Opening")')
).not.toBeVisible();
