...
test.describe("Admin password management", () => {
  ...
  test("change password limits bcrypt work per authenticated principal", async ({
    adminAPI,
    createAdmin,
  }) => {
    const admin = await createAdmin();
    let latestPassword = admin.password;
    for (let attempt = 0; attempt < 5; attempt += 1) {
      latestPassword = `RateLimited${attempt}!${randomUUID()}-password`;
      expect(
        (
          await adminAPI.post(
            "/change-password",
            { new_password: latestPassword },
            { token: admin.sessionToken },
          )
        ).status(),
      ).toBe(204);
    }
    const limited = await adminAPI.post(
      "/change-password",
      { new_password: `Rejected!${randomUUID()}-password` },
      { token: admin.sessionToken },
    );
    await expectProblem(
      limited,
    ...
    await adminAPI.passwordSession(admin.emailAddress, latestPassword);
  });

  test("password-reset request rate limiting has retry metadata", async ({
    adminAPI,
    ownedEmail,
  }) => {
    ...
    const limited = await adminAPI.post("/request-password-reset", {
      email_address,
    });
    await expectProblem(
      limited,
