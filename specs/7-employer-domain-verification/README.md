...
1.  **Root Domain Exclusivity**: Identity is tied to the **Root Domain** (e.g., `acme.com`). Subdomains (e.g., `jobs.acme.com`) are **invalid** for employer identity.
    - _Rationale_: Prevents fragmentation of a single company into multiple employer accounts.
...
**Note**: "Suspended" is an _Account Level_ status, often triggered if `FAILING_GRACE` expires. The _Domain_ strictly tracks DNS status.
