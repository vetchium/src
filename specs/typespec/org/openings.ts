...
export const ERR_EMPLOYMENT_TYPE_INVALID =
	"employment_type must be 'full_time', 'part_time', 'contract', or 'internship'";
export const ERR_WORK_LOCATION_TYPE_REQUIRED = "work_location_type is required";
export const ERR_WORK_LOCATION_TYPE_INVALID =
	"work_location_type must be 'remote', 'on_site', or 'hybrid'";
...
export const ERR_MAX_YOE_LESS_THAN_MIN =
	"max_yoe must be greater than or equal to min_yoe";
export const ERR_MIN_EDUCATION_LEVEL_INVALID =
	"min_education_level must be 'not_required', 'bachelor', 'master', or 'doctorate'";
export const ERR_SALARY_MIN_REQUIRED =
	"salary_min_amount is required if salary is provided";
export const ERR_SALARY_MAX_REQUIRED =
	"salary_max_amount is required if salary is provided";
export const ERR_SALARY_CURRENCY_REQUIRED =
	"salary_currency is required if salary is provided";
export const ERR_SALARY_MIN_POSITIVE =
	"salary_min_amount must be greater than 0";
export const ERR_SALARY_MAX_INVALID =
	"salary_max_amount must be greater than or equal to salary_min_amount";
export const ERR_SALARY_CURRENCY_INVALID =
	"salary_currency must be a 3-character ISO 4217 code";
export const ERR_SALARY_PARTIAL =
	"either all salary fields or none must be provided";
export const ERR_SALARY_MIN_AMOUNT_REQUIRED =
	"salary_min_amount is required if any salary field is provided";
export const ERR_SALARY_MAX_AMOUNT_REQUIRED =
	"salary_max_amount is required if any salary field is provided";
export const ERR_NUMBER_OF_POSITIONS_REQUIRED =
...
export const ERR_OPENING_NUMBER_INVALID =
	"opening_number must be a positive integer";
...
function validateSalary(
	...
	if (!salary.currency) {
		errs.push(
			newValidationError("salary.currency", ERR_SALARY_CURRENCY_REQUIRED)
		);
	} else if (!isValidISO4217(salary.currency)) {
		errs.push(
			newValidationError("salary.currency", ERR_SALARY_CURRENCY_INVALID)
		);
	}
}

export function validateCreateOpeningRequest(
	...
	if (!r.employment_type) {
		errs.push(
			newValidationError("employment_type", ERR_EMPLOYMENT_TYPE_REQUIRED)
		);
	} else if (
		...
		errs.push(
			newValidationError("employment_type", ERR_EMPLOYMENT_TYPE_INVALID)
		);
	}

	if (!r.work_location_type) {
		errs.push(
			newValidationError("work_location_type", ERR_WORK_LOCATION_TYPE_REQUIRED)
		);
	} else if (!["remote", "on_site", "hybrid"].includes(r.work_location_type)) {
		errs.push(
			newValidationError("work_location_type", ERR_WORK_LOCATION_TYPE_INVALID)
		);
	}
	...
	if (
		r.max_yoe !== undefined &&
		(r.max_yoe < YOE_MIN + 1 || r.max_yoe > YOE_MAX)
	) {
	...
	if (
		r.min_education_level &&
		!["not_required", "bachelor", "master", "doctorate"].includes(
			r.min_education_level
		)
	) {
		errs.push(
			newValidationError("min_education_level", ERR_MIN_EDUCATION_LEVEL_INVALID)
		);
	}
	...
	if (!r.number_of_positions) {
		errs.push(
			newValidationError(
				"number_of_positions",
				ERR_NUMBER_OF_POSITIONS_REQUIRED
			)
		);
	} else if (
		r.number_of_positions < 1 ||
		r.number_of_positions > POSITIONS_MAX
	) {
		errs.push(
			newValidationError("number_of_positions", ERR_NUMBER_OF_POSITIONS_INVALID)
		);
	}
	...
	if (
		r.internal_notes !== undefined &&
		r.internal_notes.length > INTERNAL_NOTES_MAX
	) {
		errs.push(
			newValidationError("internal_notes", ERR_INTERNAL_NOTES_TOO_LONG)
		);
	}
...
export function validateUpdateOpeningRequest(
...
export function validateRejectOpeningRequest(
	r: RejectOpeningRequest
): ValidationError[] {
	const errs: ValidationError[] = [];
	...
	if (!r.rejection_note) {
		errs.push(
			newValidationError("rejection_note", ERR_REJECTION_NOTE_REQUIRED)
		);
	} else if (r.rejection_note.length > REJECTION_NOTE_MAX) {
		errs.push(
			newValidationError("rejection_note", ERR_REJECTION_NOTE_TOO_LONG)
		);
	}
