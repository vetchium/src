...
export const ERR_INVALID_COUNTRY_CODE =
...
// Validates a single country code; returns error string or null
export function validateCountryCode(code: string): string | null {
	if (!VALID_COUNTRY_CODES.has(code)) {
		return ERR_INVALID_COUNTRY_CODE;
	}
	return null;
}

// Validates a slice of country codes; returns ValidationErrors for any invalid entry
export function validateCountryCodes(
	...
	for (const code of codes) {
		const err = validateCountryCode(code);
		if (err) {
			errs.push(
				newValidationError(field, `invalid country code "${code}": ${err}`)
			);
			break;
		}
	}
